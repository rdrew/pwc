...
END;
`
const uidArgs = [{name: 'shard_id', type: 'int4', default: 1}]
...
*/
export default async function createUID (knex, opt = emptyObj) {
  invariant(
    knex !== void 0,
  ...
  const schemaName = getSchemaName(knex)
  const key = JSON.stringify({name, epoch, seqCache, seqName, schemaName})

  if (cache[key] === void 0) {
    await createUIDSeq(knex, name, seqCache)
    cache[key] =
      func(knex, name)
        .body(body, {epoch, seqName: `${schemaName}.${seqName}`})
        .args(uidArgs)
        .returns('int8')
  }
...
export function createUIDSeq (knex, name, cacheSize = 1000) {
  return (
    sequence(knex, `${name}_seq`)
      .ifNotExists()
      .noMinvalue()
      .noMaxvalue()
      .cache(cacheSize)
...
export async function dropUID (knex, name = 'guff_uid') {
  const dropped = dropFunc(knex, name).args(uidArgs).ifExists().cascade()
  await dropSequence(knex, name).ifExists().cascade()
  return dropped
}
...
export function createKnownUIDTable (knex) {
  return createTable(
    knex,
    'guff_known_uid', [
      column('uid', 'bigInteger').defaultTo(null).primary(),
...
END;
`
const getKnownInsertName = (table, columnName) => `guff_insert_${table}_${columnName}`
const getKnownDeleteName = (table, columnName) => `guff_delete_${table}_${columnName}`

export async function createKnownUIDTrigger (knex, table, columnName) {
  const insertName = getKnownInsertName(table, columnName)
  const insertFn = func(knex, insertName, getInsertKnownUIDFunc(columnName)).returnsTrigger()
  const deleteName = getKnownDeleteName(table, columnName)
  const deleteFn = func(knex, deleteName, getDeleteKnownUIDFunc(columnName)).returnsTrigger()
  await Promise.all([insertFn, deleteFn])
  return await Promise.all([
    trigger(insertFn).on(table).eachRow().after().insert(),
    trigger(deleteFn).on(table).eachRow().after().delete(),
  ])
}

export async function dropKnownUIDTrigger (knex, table, columnName) {
  const insertName = getKnownInsertName(table, columnName)
  const droppedInsert = dropFunc(knex, insertName).ifExists().cascade()
  const deleteName = getKnownDeleteName(table, columnName)
  const droppedDelete = dropFunc(knex, deleteName).ifExists().cascade()
  await Promise.all([
    dropTrigger(knex, `${insertName}_trigger`).ifExists().on(table),
    dropTrigger(knex, `${deleteName}_trigger`).ifExists().on(table),
  ])
